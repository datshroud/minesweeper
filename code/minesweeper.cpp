...
void Minesweeper::setup(){
    ...
    gFont = TTF_OpenFont("../code/res/font.ttf", 20);
    if (!gFont){
        cout << "TTF_OpenFont Error: " << TTF_GetError() << endl;
        return; 
    }
    bFont = TTF_OpenFont("../code/res/font.ttf", 24);
    if (!bFont){
        cout << "TTF_OpenFont Error: " << TTF_GetError() << endl;
        return; 
    }

    greenFlag = IMG_LoadTexture(renderer, "../code/res/img/green_flag.png");
    if (!greenFlag) {
        cout << "Can't load image: " << IMG_GetError() << endl;
        return;
    }
    blueFlag = IMG_LoadTexture(renderer, "../code/res/img/blue_flag.png");
    if (!blueFlag){
        cout << "Can't load image: " << IMG_GetError() << endl;
        return;
    }
    mine = IMG_LoadTexture(renderer, "../code/res/img/mine.png");
    if (!mine){
        cout << "Can't load image: " << IMG_GetError() << endl;
        return;
    }
    youLose = IMG_LoadTexture(renderer, "../code/res/img/you_lose.png");
    if (!youLose){
        cout << "Can't load image yL: " << IMG_GetError() << endl;
        return;
    }
    youWin = IMG_LoadTexture(renderer, "../code/res/img/you_win.png");
    if (!youWin){
        cout << "Can't load image yW: " << IMG_GetError() << endl;
        return;
    }
    dropdown = IMG_LoadTexture(renderer, "../code/res/img/dropdown.png");
    if (!dropdown){
...
void Minesweeper::InitGrid(int level){
    this->level = level;
...
void Minesweeper::revealTile(int x, int y){
...
// tao render cho game
void Minesweeper::renderGrid(bool &running) {
    int width, height;
...
void Minesweeper::showMenu(bool &running) {
...
void Minesweeper::showBomb(int x, int y){
...
//ham cam co
void Minesweeper::flagged(int i, int j){
    int sz = 40;
    SDL_Rect tileRect = { sz * j, sz * i + 60, sz, sz };
